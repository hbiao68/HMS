...
class NewsController extends BaseController{
    ...
    public function __construct(){
        parent::__construct();
        $this->parent_mark = "news";
        $this->assign('parent_mark',$this->parent_mark);
        $this->obj = new \Common\Model\NewsModel();
        $this->catobj = new \Common\Model\NewsCatModel();
        $this->navPath = array(array('url' => U('News/index'), 'desc' => '新闻管理'));
    }
    ...
    */
    public function index(){
        ...
        //头部路径
        $navPath = array_merge($this->navPath, array(array('url' => U('News/index'), 'desc' => '新闻列表','status' => 1)));
    ...
    */
    public function add(){
        if (\Common\Common\submitcheck('submit')){
            $post = I('post.');
            if (empty($post['name'])){
                $this->error('功能名称不能为空',U('News/catAdd'));
            }else{
                $info = $this->obj->checkParam("name",$post['name']);
                if ($info){
                    $this->error("该名称已存在",U('News/add'));
                }
            }
            $post_file = $_FILES;
            ...
            $navPath = array_merge($this->navPath, array(array('url' => U('News/add'), 'desc' => '添加新闻','status' => 1)));
            $this->assign('navPath', $navPath);
            $this->assign('cat',$cat);
            $this->display();
        }
    ...
    */
    public function edit(){
        if (\Common\Common\submitcheck('submit')){
            ...
            if (empty($data['name'])){
                $this->error('名称不能为空',U('News/catEdit',array('id'=>$id)));
            }else{
                $str = "id <> $id";
                $info = $this->obj->checkParam("name",$data['name'],$str);
                if ($info){
                    $this->error("该名称已存在",U('News/edit',array('id'=>$id)));
                }else {
                    $post_file = $_FILES;
                    if (!empty($post_file['img']['name'])){
                        $file = parent::fileUplpad();
            ...
            $navPath = array_merge($this->navPath, array(array('url' => U('News/edit'), 'desc' => '修改新闻','status' => 1)));
            $this->assign('navPath', $navPath);
    ...
    */
    public function catList(){
    	...
    	$this->assign('page', $results['pageShow']); // 赋值分页输出
        //头部路径
        $navPath = array_merge($this->navPath, array(array('url' => U('News/catList'), 'desc' => '新闻分类列表','status' => 1)));
        $this->assign('navPath', $navPath);
        $this->display();
    }
    /**
    ...
    public function catAdd() {
        if (\Common\Common\submitcheck('submit')){
            ...
            if (empty($post['name'])){
                ...
                if ($info){
                    $this->error("该名称已存在",U('News/catAdd'));
                }
            }
            $result = $this->catobj->add($post);
            if ($result) {
                $this->success("添加成功", U('News/catList'));
            } else {
                $this->error("添加失败", U('News/catList'));
            }
        }else {
            $info = $this->catobj->get(array('pid'=>0));
            $navPath = array_merge($this->navPath, array(array('url' => U('News/catAdd'), 'desc' => '添加新闻分类','status' => 1)));
            $this->assign('navPath', $navPath);
    ...
    */
    ...
    if (\Common\Common\submitcheck('submit')){
           ...
           $id = I('post.id');
           if (empty($id)){
               $this->error('参数错误',U('News/catList'));
           }
           if (empty($data['name'])){
               $this->error('分类名称不能为空',U('News/catEdit',array('id'=>$id)));
           }else{
               $str = "id <> $id";
               $info = $this->catobj->checkParam("name",$data['name'],$str);
               if ($info){
                   $this->error("该名称已存在",U('News/catEdit',array('id'=>$id)));
               }else {
                   $result = $this->catobj->where("id = $id")->save($data);
                   if ($result !== false){
                       $this->success("修改成功",U('News/catList'));
                   }else{
                       $this->error("修改失败",U('News/catList'));
                   }
               }
           }
        }else{
            ...
            $navPath = array_merge($this->navPath, array(array('url' => U('News/catedit'), 'desc' => '修改新闻分类','status' => 1)));
            $this->assign('navPath', $navPath);
            $this->display();
        }
    }
