            ...
            <header class="panel-heading">
                	导航列表
                        <span class="tools pull-right">
                        	<a href="<?php echo U('MenuWeb/add');?>" class="fa">添加导航</a>
                ...
                <section id="unseen">
                    <table class="table table-bordered table-striped table-condensed">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php if(($vo["tid"]) == "1"): ?>顶部<?php endif; if(($vo["tid"]) == "2"): ?>底部<?php endif; ?></td>
                            <td><?php echo ($vo["url"]); ?></td>
                            <td><?php if(($vo["status"]) == "1"): ?>可用<?php else: ?>不可用<?php endif; ?></td>
                            <td><?php echo ($vo["sort"]); ?></td>
                            <td class="numeric"><?php if(empty($vo["createtime"])): else: echo (date("Y-m-d H:i:s",$vo["createtime"])); endif; ?></td>
                            <td class="numeric">
                            	<a href="<?php echo U('MenuWeb/edit',array('id'=>$vo['id']));?>" class="edit">修改</a> | 
                            	<?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('MenuWeb/editStatus',array('id'=>$vo['id'],'status'=>2));?>" class="edit">标识为不可用</a><?php endif; ?>
                            	<?php if(($vo["status"]) == "2"): ?><a href="<?php echo U('MenuWeb/editStatus',array('id'=>$vo['id'],'status'=>1));?>" class="edit">标识为可用</a><?php endif; ?>
