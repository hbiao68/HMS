	...
	<div class="col-lg-12">
		<section class="panel">
			<div class="panel-body">
				<form class="form-inline" role="form" method="get" action="<?php echo U('AuthRule/index');?>">
					<div class="form-group">
						<label class="sr-only" for="title">title</label> 
						<input type="text" class="form-control" id="title" name="title" placeholder="功能名称" value="<?php echo ($_GET['title']); ?>" />
					</div>
					<div class="form-group">
						<label class="sr-only" for="url">url</label> 
						<input type="text" class="form-control" id="url" name="url" placeholder="URL" value="<?php echo ($_GET['url']); ?>" />
					</div>
					<div class="form-group">
						<label class="sr-only" for="pid">pid</label> 
						<select class="form-control" name="pid">
							<option value="">请选择上级菜单</option>
                        	<?php if(is_array($top)): $i = 0; $__LIST__ = $top;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if(($_GET['pid']) == $vo["id"]): ?>selected="selected"<?php endif; ?>><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                        </select>
					</div>
            ...
            <header class="panel-heading">
                	系统功能列表
                        <span class="tools pull-right">
                        	<a href="<?php echo U('AuthRule/add');?>" class="fa">添加系统功能</a>
                ...
                <section id="unseen">
                    <table class="table table-bordered table-striped table-condensed">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td class="numeric"><?php if(($vo["create_time"]) == "0"): else: echo (date("Y-m-d H:i:s",$vo["create_time"])); endif; ?></td>
                            <td><?php echo ($vo["status"]); ?></td>
                            <td class="numeric"><?php echo ($vo["is_menu"]); ?></td>
                            <td class="numeric"><?php echo ($vo["mark"]); ?></td>
                            <td class="numeric"><a href="<?php echo U('AuthRule/edit',array('id'=>$vo['id']));?>" class="edit">修改</a></td>
